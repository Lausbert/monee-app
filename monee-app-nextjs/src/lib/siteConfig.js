const siteConfig = {  site_url: "https://monee-app.com",
  ios_app_id: "1617877213",
  appstore_link: "https://apps.apple.com/app/id1617877213", // Direct App Store link
  playstore_link: "https://play.google.com/store/apps/details?id=app.monee",
  app_icon: "/assets/appicon.webp", // Path relative to public directory
  enable_smart_app_banner: true,
  your_name: "Stephan Lerner",
  your_link: "https://www.linkedin.com/in/stephan-lerner-301ba091/",
  your_city: "Hamburg",
  email_address: "info@monee-app.com",
  facebook_username: "",
  instagram_username: "",
  twitter_username: "",
  github_username: "Lausbert",
  product_hunt_link: "https://www.producthunt.com/products/monee",
  youtube_username: "",
  topbar_color: "#000000",
  topbar_transparency: 0.0,
  topbar_title_color: "#ffffff",
  cover_image: "/assets/headerimage.webp", // Path relative to public directory
  cover_overlay_color: "#363b3d",
  cover_overlay_transparency: 0.0,
  cover_overlay_start_position: "0%",
  cover_overlay_end_transparency: 0.5,
  body_background_color: "#ffffff",
  link_color: "#3CBCB8",
  banner_color: "#3CBCB8",
  banner_transparency: 0.8,
  app_title_color: "#ffffff",
  app_description_color: "#ffffff",
  review_card_background_color: "#f7f7f7",
  feature_title_color: "#000000",
  feature_text_color: "#666666",
  feature_icons_foreground_color: "#3CBCB8",
  feature_icons_background_color: "#e6e6e6",
  social_icons_foreground_color: "#666666",
  social_icons_background_color: "#e6e6e6",
  footer_text_color: "#666666",
  default_lang: "en", // Keep for backward compatibility
  defaultLanguage: "en", // Standard format
  languages: ["en", "de", "fr", "es", "pt", "it", "ru", "hi", "el", "tr"],
  // Locales where blog is enabled. New languages can be added without blog support by
  // omitting them from this list. All current languages have blog enabled.
  blog_enabled_locales: ["en", "de", "fr", "es", "pt", "it", "ru", "hi"],
  app_name: "Monee - Budget & Expense Tracker",
  app_description: "Money Tracking App",
  app_keywords: "money tracking, finance, expenses, budgeting",  
  footer_links: [
    {
      title: "Blog",
      url: "/blog/",
      title_key: "titles.blog"
    },
    {
      title: "Imprint",
      url: "/imprint/",
      title_key: "titles.imprint"
    },
    {
      title: "Privacy Policy", 
      url: "/privacy/",
      title_key: "titles.privacy"
    }  ],
  mailerlite_form_id: "153131010294286151", // MailerLite form ID from the URL
  mailerlite_universal: "1489745", // MailerLite universal ID from the URL
};

export default siteConfig;
